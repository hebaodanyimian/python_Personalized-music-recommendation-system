...
<div>
	<div :style='{"padding":"20px 20px 10px","margin":"10px auto","borderColor":"#33","borderRadius":"0","background":"#fff","borderWidth":"0 0 2px","width":"90%","backgroundSize":"100% 40%","backgroundPosition":"bottom center","borderStyle":"solid","backgroundRepeat":"no-repeat"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"16px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/gequpaixingbang?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"width":"55%","padding":"0 20px","position":"relative","background":"#fff","display":"flex","order":"2"}'>
			...
			<div class="info" :style='{"padding":"0 10px","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>歌名</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.songname}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>播放量</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.bofangliang}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>更新时间</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.gengxinshijian}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"#fff","width":"auto","lineHeight":"40px","fontSize":"15px","height":"40px"}' v-if="btnAuth('gequpaixingbang','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('gequpaixingbang','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('gequpaixingbang','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
            if (res.data.code == 0) {
              this.detail = res.data.data;
				this.title = this.detail.gedanmingcheng;
      ...
      storeup(type) {
        if (type == 1 && !this.isStoreup) {
          ...
          this.$http.post('storeup/add', this.storeupParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('gequpaixingbang/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'gequpaixingbang', userid: localStorage.getItem('frontUserid')}}).then(res => {
    ...
    getThumbsupOrCrazilyStatus() {
        if(localStorage.getItem("frontToken")) {
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 21, refid: this.detail.id, tablename: 'gequpaixingbang', userid: localStorage.getItem('frontUserid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
            ...
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 22, refid: this.detail.id, tablename: 'gequpaixingbang', userid: localStorage.getItem('frontUserid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
      ...
      sizeChange(size){
		...
		backClick(){
			...
			this.$router.push({path: '/index/gequpaixingbang', query: params});
		},
		// 下载
		download(file){
			if(!file) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/gequpaixingbangAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此歌曲排行榜？')
			  .then(_ => {
			    this.$http.post('gequpaixingbang/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
