...
<div>
	<div :style='{"padding":"20px 20px 10px","margin":"10px auto","borderColor":"#33","borderRadius":"0","background":"#fff","borderWidth":"0 0 2px","width":"90%","backgroundSize":"100% 40%","backgroundPosition":"bottom center","borderStyle":"solid","backgroundRepeat":"no-repeat"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"16px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/discussgequxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"width":"55%","padding":"0 20px","position":"relative","background":"#fff","display":"flex","order":"2"}'>
			...
			<div class="info" :style='{"padding":"0 10px","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>关联表id</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.refid}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>用户名</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.nickname}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>评论内容</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.content}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>回复内容</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.reply}}</div>
				</div>
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"#fff","width":"auto","lineHeight":"40px","fontSize":"15px","height":"40px"}' v-if="btnAuth('discussgequxinxi','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discussgequxinxi','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discussgequxinxi','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
    ...
    methods: {
      ...
      sizeChange(size){
		...
		backClick(){
			...
			this.$router.push({path: '/index/discussgequxinxi', query: params});
		},
		// 下载
		download(file){
			if(!file) {
      ...
      getDiscussList(page) {
        this.$http.get('discussdiscussgequxinxi/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussdiscussgequxinxi/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussdiscussgequxinxi/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		  ...
		  this.$http.post('discussgequxinxi/update',this.detail).then(res=>{
		...
		// 修改
		editClick(){
			this.$router.push(`/index/discussgequxinxiAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此歌曲信息评论表？')
			  .then(_ => {
			    this.$http.post('discussgequxinxi/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
