...
<div>
	<div :style='{"padding":"20px 20px 10px","margin":"10px auto","borderColor":"#33","borderRadius":"0","background":"#fff","borderWidth":"0 0 2px","width":"90%","backgroundSize":"100% 40%","backgroundPosition":"bottom center","borderStyle":"solid","backgroundRepeat":"no-repeat"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"16px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/gequleixing?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"width":"55%","padding":"0 20px","position":"relative","background":"#fff","display":"flex","order":"2"}'>
			...
			<div class="info" :style='{"padding":"0 10px","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"#fff","width":"auto","lineHeight":"40px","fontSize":"15px","height":"40px"}' v-if="btnAuth('gequleixing','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('gequleixing','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('gequleixing','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
    ...
    methods: {
      ...
      sizeChange(size){
		...
		backClick(){
			...
			this.$router.push({path: '/index/gequleixing', query: params});
		},
		// 下载
		download(file){
			if(!file) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/gequleixingAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此歌曲类型？')
			  .then(_ => {
			    this.$http.post('gequleixing/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
