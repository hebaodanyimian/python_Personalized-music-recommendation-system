...
def yonghu_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        for item in msg['data']['list']:
            item['youxiang'] = Common.desensitization(Common,item['youxiang'],'邮')
            item['shoujihao'] = Common.desensitization(Common,item['shoujihao'],'手')
        return JsonResponse(msg, encoder=CustomJsonEncoder)
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="是"  and  "clicknum"  in yonghu.getallcolumn(yonghu,yonghu):
            ...
            if ret!=None:
                msg['code'] = crud_error_code
                msg['msg'] = ret
        msg['data']['youxiang'] = Common.desensitization(Common,msg['data']['youxiang'],'邮')
        msg['data']['shoujihao'] = Common.desensitization(Common,msg['data']['shoujihao'],'手')
        return JsonResponse(msg, encoder=CustomJsonEncoder)
