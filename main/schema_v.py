...
def schemaName_file_upload(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if file:
            ...
            filePath = os.path.join(os.getcwd(), "templates/upload", file_name)
...
def schemaName_file_download(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        filePath = os.path.join(os.getcwd(), "templates/upload", filename)
        print("filePath===========>", filePath)
        if '是' in hasHadoop or 'spark' in hasHadoop:
            try:
                client = Client("http://127.0.0.1:50070/")
...
def schemaName_spider(request, tableName):
    if request.method in ["POST", "GET"]:
        ...
        # Linux
        cmd = "cd /yykj/python/9999/spider${spiderSchemaName} && scrapy crawl "+tableName+"Spider -a databaseName=django26tm7521"
        # Windows
        # cmd = "cd C:\\test1\\spider && scrapy crawl " + tableName + "Spider -a databaseName=django26tm7521"
        os.system(cmd)
